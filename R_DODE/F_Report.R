...
report.tickets.initialize <- function(tickets, item_symbol.mapping, support.symbols.table, timeframe = 'M1', format.digit = 2, db = c('mysql.old', 'mysql.new', 'influxdb')) {
  # ''' init report tickets '''
  # 2016-08-16: TESTING
...
report.tickets.add.symbol <- function(tickets, item_symbol.mapping) {
...
report.tickets.support.tickets <- function(original.tickets) {
...
report.tickets.choose.group <- function(tickets, pattern) {
...
report.tickets.recalculate <- function(tickets, support.symbols.table, timeframe = 'M1', format.digit = 2, db = c('mysql.old', 'mysql.new', 'influxdb')) {
...
report.info.currency <- function(report.info, default = 'USD') {
...
report.info.leverage <- function(report.info, default = 100) {
...
report.tickets.items <- function(tickets) {
  # ''' get items of tickets '''
  # 2016-08-12: Done
  # 2016-08-16:tickets from list to one data.frame
  #@2016-08-12
  levels(factor(toupper(tickets$Item)))
} # 2016-08-16: Done

report.item_symbol.mapping <- function(items, support.symbols = rownames(SYMBOLS)) {
  # ''' item to symbol '''
  # 2016-08-12: Done
  if (length(items) == 0) {
    return(NULL)
  ...
  require(stringr) # for str_detect()
...
report.symbols.table <- function(symbols, currency = 'USD', leverage = 100, support.symbols.table = SYMBOLS) {
...
report.symbols.table.tick.value <- function(symbols, symbols.table, currency = 'USD') {
...
report.symbols.table.margin.required <- function(symbols, symbols.table, currency = 'USD', leverage = 100) {
...
report.parameter <- function(capital.initialize.setting, capital.hypothesis, capital.leverage, tickvalue.timeframe, profit.digit, db, score.return.table, score.maxdd.table, score.weight, score.level.table) {
  # ''' parameter '''
  # 2016-08-19: TESTING
  list(
...
report.period <- function(trades.after.hypothesis) {
  # ''' period ''
  # 2016-08-19: Done
...
build.symbol <- function(currency1, currency2, support.symbols = rownames(SYMBOLS)) {
...
calculate.profit.from.tickvalue_volume_pips <- function(tickvalue, volume, pips, format.digit = 2) {
  # ''' calculate profit from para: tickvalue, volume, pips '''
  # 2016-08-15: Done
  round(tickvalue * volume * pips, format.digit)
} # 2016-08-15: Done

calculate.profit.from.tickets.columns <- function(symbol, time, volume, type, open.price, close.price, support.symbols.table, timeframe = 'M1', format.digit = 2, db = c('mysql.old', 'mysql.new', 'influxdb')) {
...
calculate.profit.from.tickets <- function(tickets, support.symbols.table, timeframe = 'M1', format.digit = 2, db = c('mysql.old', 'mysql.new', 'influxdb')) {
...
calculate.tickvalue <- function(symbols, time, timeframe = 'M1', support.symbols.table, db = c('mysql.old', 'mysql.new', 'influxdb')) {
...
calculate.pips <- function(symbol, type, open.price, close.price, support.symbols.table) {
  # ''' calculate pips '''
  # 2016-08-15: TESTING
