# ''' load price data functions '''
# 2016-08-22: Create
# 2016-08-23: Name & Vectorize

price.data.required <- function(symbols, from, to, timeframe, db = c('mysql.old', 'mysql.new', 'influxdb')) {
  # ''' get need price data '''
  # 2016-08-23: TESTING
  if (length(db) > 1) {
    db <- 'mysql.old'
  ...
  required.prices <- lapply(load.prices, function(data.list) {
...
load.price.data.from.local_db <- function(symbol, from, to, timeframe, local.data.list, db = c('mysql.old', 'mysql.new', 'influxdb')) {
...
load.price.data.from.local <- function(symbol, from, to, timeframe, local.data.list) {
...
load.price.data.from.db <- function(symbol, from, to, timeframe, db = c('mysql.old', 'mysql.new', 'influxdb')) {
...
price.data.open <- function(symbol, time, timeframe = 'M1', db = c('mysql.old', 'mysql.new', 'influxdb')) {
  # ''' price data open '''
  # 2016-08-23: TESTING
  if (length(db) > 1) {
    db <- 'mysql.old'
...
db.mysql.query <- function(sql, host = '192.168.1.9', port = 3306, username = 'mtk_bj_db', password = 'mtk_bj_db', dbname = 'auto_history') {
  # ''' mysql query '''
  # 2016-08-23: TESTING
  require(RMySQL)
  sql.len <- length(sql)
...
db.mysql.old.query.price.data.open <- function(symbol, time, timeframe = 'M1') {
...
load.price.data.from.db.mysql.old <- function(symbol, from, to, timeframe) {
...
db.mysql.new.query.price.data.open <- function(symbol, time, timeframe = 'M1') {
...
load.price.data.from.db.mysql.new <- function(symbol, from, to, timeframe) {
...
db.influxdb.query.price.data.open <- function(symbol, time, timeframe = 'M1') {
  # ''' influxdb query: open '''
  # 2016-08-23: TESTING
...
load.price.data.from.db.influxdb <- function(symbol, from, to, timeframe) {
  ...
  serie.values <- lapply(series$values, function(x) {
    data.frame(
      stringsAsFactors = F,
      x[[1]],
      Open = x[[2]],
...
db.influxdb.query <- function(query, host = '192.168.1.155', port = 8086, username = 'strategy', password = 'strategy', database = 'auto_history', time_precision=c("s", "m", "u")) {
  # ''' influxdb query '''
  # 2016-08-23: TESTING
  require(httr)
  require(rjson)
...
timeframe.secs <- function(timeframe) {
  # ''' time frame secs '''
  # 2016-08-23: Done
