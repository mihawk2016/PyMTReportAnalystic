...
report.output <- function(report, timeframe = 'H1', dir = './Output', comment = F) {
  # ''' report output '''
  # 2016-08-18: Coding
  require(rmarkdown)
...
report.output.file <- function(info, timeframe, dir, comment, envir) {
  # ''' report output file '''
  # 2016-08-18: TESTING
...
report.output.file.name <- function(info, timeframe = 'H1') {
...
report.output.info <- function(info) {
  # ''' head for output '''
  # 2016-08-11: Done
  require(htmlTable)
...
report.output.portfolio.result <- function(portfolio) {
...
report.output.portfolio.returns <- function(portfolio, trade.days) {
...
report.output.portfolio.trades <- function(portfolio, trade.days) {
...
report.output.portfolio.summary <- function(portfolio, timeseries, capital) {
...
report.output.symbols.profit_loss <- function(symbols) {
  # ''' symbol for output '''
  # 2016-08-25: Coding
  require(htmlTable)
...
symbol.statistics.profit_loss.in.one.row <- function(symbol) {
...
report.output.symbols.summary <- function(symbols, timeseries, report.capital) {
  # ''' symbol for output '''
  # 2016-08-25: Coding
  require(htmlTable)
...
symbol.statistics.summary.in.one.row <- function(symbol, report.capital) {
...
symbol.statistics.mdd.in.one.row <- function(timeseries, report.capital) {
...
trades.output <- function(trades.after.hypothesis, timeseries) {
  # '''
  # 2016-08-29: Coding
  require(htmlTable)
  symbol.timeseries <- timeseries$Symbol
  trades.floatings <- do.call(rbind, lapply(symbol.timeseries, function(symbol.timeserie) {
    symbol.trades <- symbol.timeserie$Trades
    do.call(rbind, lapply(symbol.trades, function(symbol.trade) {
...
report.output.balance.serie <- function(timeseries, money.trades, period) {
  # ''' plot blance serie '''
  # 2016-08-29: Coding
  require(ggplot2)
...
report.output.score <- function(portfolio, trade.period, timeseries, capital, return.table, mdd.table, weight, level.table) {
  require(ggplot2)
  require(grid)
  return <- portfolio$Return_Sharpe$ReturnR
...
report.output.equity_volume <- function(timeseries, capital) {
  require(ggplot2)
  require(reshape2)
  require(grid)
  portfolio.timeserie <- timeseries$Portfolio
  ...
  equity.table <- do.call(cbind, lapply(symbol.timeserie, function(symbol) {
  ...
  volume.table <- do.call(cbind, lapply(symbol.timeserie, function(symbol) {
    symbol.table <- symbol$Symbol
...
report.output.margin <- function(timeseries, capital) {
  require(ggplot2)
  require(grid)
  portfolio.timeserie <- timeseries$Portfolio
...
report.capital <- function(parameter) {
  # ''' get capital '''
  # 2016-08-25: TESTING
...
report.calendar.return <- function(timeseries, capital, digit = 2, percent = F) {
...
report.calendar.mdd <- function(timeseries, capital, digit = 2, percent = F) {
  timeserie.portfolio <- timeseries$Portfolio
  equity.portfolio <- timeserie.portfolio$Balance.Delta + timeserie.portfolio$Floating + capital
  mdd.table <- calendar.table(equity.portfolio, fun = function(x) {
...
report.evaluation <- function(portfolio, trade.days, timeseries, capital) {
  require(htmlTable)
  timeserie.portfolio <- timeseries$Portfolio
  equity.portfolio <- timeserie.portfolio$Balance.Delta + timeserie.portfolio$Floating + capital
  returns.portfolio <- Return.calculate(equity.portfolio)
...
cal.scores <- function(yearly.return, max.dd, return.table, mdd.table, weight, level.table) {
...
calendar.table <- function(xts, fun, digit = 2, percent = F, name = '') {
  xts <- na.omit(xts)
  ...
  sapply(as.character(year.labels), function(year) {
    value <- fun(xts[year])
    if (percent) {
      value <- value * 100
    }
